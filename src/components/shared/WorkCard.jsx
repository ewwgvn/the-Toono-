    ...
    return (
      <div style={{ marginBottom: 0, background: "#FFFFFF", borderBottom: `1px solid ${T.borderLight}` }}>
        ...
        <div onClick={handleDoubleTap} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}
          style={{ width: "100%", aspectRatio: imgs.length > 0 ? (imgRatio || "1") : "4/5", background: T.s2, cursor: "pointer", position: "relative", overflow: "hidden", userSelect: "none" }}>
          {imgs.length > 0 ? (
            imgs[imgIdx].startsWith("data:") ? (
        ...
        <div style={{ padding: "10px 16px 2px", display: "flex", alignItems: "center" }}>
          <div style={{ display: "flex", gap: 16, alignItems: "center", flex: 1 }}>
            <button type="button" onClick={(e) => { e.stopPropagation(); onToggleLike && onToggleLike(w.id); }}
              style={{ background: "none", border: "none", cursor: "pointer", color: liked ? T.red : T.textH, display: "flex", padding: 2 }}>
              <IcHeart filled={liked} />
            </button>
            <button type="button" onClick={onClick} style={{ background: "none", border: "none", cursor: "pointer", color: T.textH, display: "flex", padding: 2 }}>
            ...
            <button type="button" onClick={(e) => { e.stopPropagation(); const url = `${window.location.origin}?work=${w.id}`; if (navigator.share) navigator.share({ title: w.title, url }); else if (navigator.clipboard) navigator.clipboard.writeText(url); }}
              style={{ background: "none", border: "none", cursor: "pointer", color: T.textH, display: "flex", padding: 2 }}>
              <IcShare />
            </button>
          </div>
          <button type="button" onClick={(e) => { e.stopPropagation(); onToggleSave && onToggleSave(w.id); }}
            style={{ background: "none", border: "none", cursor: "pointer", color: saved ? T.textH : T.textSub, display: "flex", padding: 2 }}>
            <IcBookmark filled={saved} />
          </button>
        </div>
        {likeCount > 0 && <div style={{ padding: "2px 16px", fontFamily: F, fontSize: 13, fontWeight: 600, color: T.textH }}>{likeCount.toLocaleString()} таалагдсан</div>}
        ...
        <div style={{ padding: "8px 16px 12px", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <input value={commentText} onChange={(e) => setCommentText(e.target.value)}
            ...
            style={{ flex: 1, background: "none", border: "none", outline: "none", fontFamily: F, fontSize: 13, color: T.textB, padding: 0 }}
          />
          {commentText.trim() && (
            <button type="button" onClick={() => { if (onComment) { onComment(w.id, commentText.trim()); setCommentText(""); } }}
              style={{ background: "none", border: "none", fontFamily: F, fontSize: 13, fontWeight: 600, color: T.textH, cursor: "pointer", padding: 0 }}>Нийтлэх</button>
          )}
        </div>
      </div>
    );
    ...
    return (
      <div onClick={onClick} style={{ display: "flex", gap: 12, padding: "12px 0", borderBottom: `1px solid ${T.borderLight}`, cursor: "pointer" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <div style={{ fontFamily: F, fontSize: 12, color: T.textSub, marginBottom: 4 }}>{w.creator}</div>
          <div style={{ fontFamily: F, fontSize: 14, fontWeight: 600, color: T.textH }}>{fmtP(w)}</div>
        </div>
      </div>
    );
  }
    ...
    <div onClick={onClick} style={{ cursor: "pointer" }}>
      ...
      {likeNum > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
          <IcHeart />
          <span style={{ fontFamily: F, fontSize: 11, color: T.textSub }}>{likeNum}</span>
        </div>
      )}
    </div>
  );
