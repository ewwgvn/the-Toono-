 ...
 *
 * Escapes structural characters used in PostgREST .or() filter syntax
 * (comma, parentheses, dot, backslash) that could allow a caller to inject
 * extra filter conditions or break query structure. Also escapes SQL LIKE
 * wildcards (%, _) to prevent pattern-matching abuse in .ilike() calls.
 *
...
export function sanitizeFilter(str) {
  ...
  return str
    .replace(/\\/g, "\\\\")  // backslash — PostgREST escape char (must come first)
    .replace(/,/g, "\\,")    // comma — .or() condition separator
    .replace(/\(/g, "\\(")   // parentheses — .or() grouping
    .replace(/\)/g, "\\)")
    .replace(/\./g, "\\.")   // dot — col.operator.value separator
    .replace(/%/g, "\\%")    // LIKE wildcard (any sequence)
    .replace(/_/g, "\\_");   // LIKE wildcard (single char)
}
...
export const DB = {
  ...
  async getWorks(filters = {}) {
    ...
    if (filters.search) { const s = sanitizeFilter(filters.search); query = query.or(`title.ilike.%${s}%,description.ilike.%${s}%`); }
  ...
  async toggleLike(userId, workId) {
    ...
    const wasLiked = !!(data && data.length > 0);
    if (wasLiked) {
      const { error } = await supabase.from("likes").delete().eq("user_id", userId).eq("work_id", workId);
      if (error) { GS.liked.add(workId); saveGS(); return; }
      await supabase.rpc("decrement_likes", { wid: workId }).catch(() => {});
    } else {
      const { error } = await supabase.from("likes").insert({ user_id: userId, work_id: workId });
      if (error) { GS.liked.delete(workId); saveGS(); return; }
  ...
  async toggleSave(userId, workId) {
    ...
    const wasSaved = !!(data && data.length > 0);
    if (wasSaved) {
      const { error } = await supabase.from("saves").delete().eq("user_id", userId).eq("work_id", workId);
      if (error) { GS.saved.add(workId); saveGS(); }
    } else {
      const { error } = await supabase.from("saves").insert({ user_id: userId, work_id: workId });
      if (error) { GS.saved.delete(workId); saveGS(); }
    }
  },
  ...
  async toggleFollow(followerId, followingId) {
    ...
    const wasFollowing = !!(data && data.length > 0);
    if (wasFollowing) {
      const { error } = await supabase.from("follows").delete().eq("follower_id", followerId).eq("following_id", followingId);
      if (error) { GS.following.add(followingId); saveGS(); }
    } else {
      const { error } = await supabase.from("follows").insert({ follower_id: followerId, following_id: followingId });
      if (error) { GS.following.delete(followingId); saveGS(); }
    }
  },
  ...
  async markConversationRead(conversationId, userId) {
    ...
    const update = {};
    if (isA) update.unread_a = 0;
    else update.unread_b = 0;
    await supabase.from("conversations").update(update).eq("id", conversationId);
  ...
  subscribeToConversation(conversationId, callback) {
    ...
    return supabase.channel(`conv:${conversationId}`)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "conversations", filter: `id=eq.${conversationId}` }, payload => callback(payload.new))
      .subscribe();
  },
  ...
  async getCreators(filters = {}) {
    ...
    if (filters.search) { const s = sanitizeFilter(filters.search); query = query.or(`name.ilike.%${s}%,field.ilike.%${s}%`); }
    if (filters.limit) query = query.limit(filters.limit);
    const { data } = await query;
    return data || [];
  },

  // All profiles (creators + buyers)
  async getAllProfiles(limit = 50) {
    if (!isSupabaseReady()) return [];
    const { data } = await supabase
      .from("profiles")
      .select("id,name,field,photo,role,bio,followers_count,rating")
...
function normalizeOrder(o) {
  ...
  const shipping = o.shipping_fee || o.shipping || 0;
  const total = o.total_price || o.price || subtotal + shipping;
  const platformFee = o.platform_fee ?? o.platformFee ?? Math.round(subtotal * 0.08);
  return {
    ...o,
    title: o.title || first.title || (items.length ? `${items.length} items` : "Захиалга"),
    creator: o.creator || "—",
    price: total,
    subtotal,
    shipping,
    platformFee,
    sellerPayout: o.seller_payout ?? o.sellerPayout ?? Math.max(0, subtotal - platformFee),
...
export async function syncFromSupabase() {
  ...
  try {
    ...
    GS.conversations = convos.map(cv => {
      ...
      const myUnread = isA ? cv.unread_a : cv.unread_b;
      return {
        id: cv.id, dbId: cv.id,
        ...
        online: false,
        unread: myUnread || 0,
        msgs: cv.last_message ? [{ id: 0, mine: false, text: cv.last_message, time: "", status: "delivered" }] : [],
      };
    });
