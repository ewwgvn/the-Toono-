...
import { IcBack, IcBookmark, IcTrash, IcFilter, IcCheck, IcCartEmpty } from "@/components/icons";
...
export default function CartScreen({ nav, refresh, goBack }) {
  ...
  const itemKey = (item) => `${item.id}-${item.size || "base"}-${item.color || "base"}`;
  const getQty = (item) => qty[itemKey(item)] || item.qty || 1;
  const subtotal = cartItems.reduce((s, it) => s + (it.price * getQty(it)), 0);
  ...
  return <div style={{ height: "100%", display: "flex", flexDirection: "column", background: T.bg }}>
    ...
    <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none", padding: "0 20px" }}>
      {cartItems.length === 0
        ...
        : <>
          {cartItems.map(item => <Crd key={itemKey(item)} style={{ marginBottom: 12, padding: 0, overflow: "hidden" }}>
            <div style={{ display: "flex", gap: 0 }}>
              ...
              <div style={{ flex: 1, padding: "12px 14px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 15, fontWeight: 700, color: T.accent }}>₮{(item.price * getQty(item)).toLocaleString()}</div>
                  {!item.digital && <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <button type="button" aria-label="Тоо багасгах" onClick={() => setQty(q => ({ ...q, [itemKey(item)]: Math.max(1, getQty(item) - 1) }))} style={{ width: 28, height: 28, borderRadius: 8, background: T.s2, border: `1px solid ${T.border}`, color: T.textH, cursor: "pointer", fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                    <span style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 14, fontWeight: 700, color: T.textH, minWidth: 16, textAlign: "center" }}>{getQty(item)}</span>
                    <button type="button" aria-label="Тоо нэмэх" onClick={() => setQty(q => ({ ...q, [itemKey(item)]: Math.min(item.stock || 99, getQty(item) + 1) }))} style={{ width: 28, height: 28, borderRadius: 8, background: T.s2, border: `1px solid ${T.border}`, color: T.textH, cursor: "pointer", fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                  </div>}
                </div>
              </div>
            </div>
            <div style={{ borderTop: `1px solid ${T.border}`, padding: "8px 14px", display: "flex", justifyContent: "space-between" }}>
              <button type="button" onClick={() => { GS.saved.add(item.id); GS.cart = GS.cart.filter(c => itemKey(c) !== itemKey(item)); saveGS(); refresh(); }} style={{ background: "none", border: "none", fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 12, color: T.accent, cursor: "pointer" }}><span style={{ display: "flex" }}><IcBookmark /></span> Хадгалах</button>
              <button type="button" onClick={() => { GS.cart = GS.cart.filter(c => itemKey(c) !== itemKey(item)); saveGS(); refresh(); }} style={{ background: "none", border: "none", fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 12, color: T.red, cursor: "pointer" }}><span style={{ display: "flex" }}><IcTrash /></span> Хасах</button>
            </div>
          </Crd>)}
          ...
          <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, padding: "12px 14px", marginBottom: 16, background: "#FFFFFF", display: "grid", gap: 8 }}>
            {["Төлбөр эскроуд хадгалагдана", "Хүргэлтийн дараа худалдан авагч баталгаажуулна", "Асуудал гарвал маргаан нээж болно"].map(text => (
              <div key={text} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ width: 18, height: 18, borderRadius: 9, background: T.accentSub, color: T.accent, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><IcCheck /></span>
                <span style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 12, color: T.textSub }}>{text}</span>
              </div>
            ))}
          </div>
        </>}
    ...
    {cartItems.length > 0 && <div style={{ padding: "14px 20px calc(20px + env(safe-area-inset-bottom, 0px))", borderTop: `1px solid ${T.border}`, background: "#FFFFFF", display: "flex", alignItems: "center", gap: 14 }}>
      <div style={{ flexShrink: 0 }}>
        <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 11, color: T.textSub, marginBottom: 1 }}>Нийт дүн</div>
        <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 19, fontWeight: 800, color: T.textH, lineHeight: 1 }}>₮{total.toLocaleString()}</div>
      </div>
      <div style={{ flex: 1 }}>
        <PBtn full onClick={() => {
          GS.cart = cartItems.map(it => ({...it, qty: getQty(it)}));
          GS.directBuyItem = null; saveGS(); nav("checkout");
        }}>Захиалах ({cartItems.length})</PBtn>
      </div>
    </div>}
  </div>;
}
