...
export default function ChatList({ nav, refresh, goBack }) {
  ...
  useEffect(() => {
    ...
    const subs = GS.conversations
      .filter(c => c.dbId)
      .map(c => DB.subscribeToConversation?.(c.dbId, (updated) => {
        const isA = updated.user_a === GS.user.id;
        const newUnread = isA ? updated.unread_a : updated.unread_b;
        const convo = GS.conversations.find(cv => cv.dbId === c.dbId);
        if (convo && newUnread !== convo.unread) {
          convo.unread = newUnread || 0;
          GS.unreadChat = GS.conversations.reduce((s, cv) => s + (cv.unread || 0), 0);
          saveGS(); refresh();
        }
      }));
    return () => { subs.forEach(s => s?.unsubscribe?.()); };
  }, [GS.conversations.length]);
  ...
  const openChat = (c) => {
    ...
    if (c.dbId && isSupabaseReady() && GS.user.id) {
      DB.markConversationRead(c.dbId, GS.user.id).catch(() => {});
    }
    nav("chatroom");
  };
