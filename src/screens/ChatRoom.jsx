...
export default function ChatRoom({ nav, refresh, goBack }) {
  ...
  React.useEffect(() => {
    ...
    const sub = DB.subscribeToMessages?.(convo.dbId, (newMsg) => {
      if (newMsg.sender_id !== GS.user.id) {
        const mapped = { id: newMsg.id, mine: false, text: newMsg.text, img: !!newMsg.image, imgSrc: newMsg.image || null, time: new Date(newMsg.created_at).getHours() + ":" + String(new Date(newMsg.created_at).getMinutes()).padStart(2, "0"), status: "delivered" };
        setMsgs(prev => [...prev, mapped]);
        if (convo) { convo.msgs = [...(convo.msgs || []), mapped]; saveGS(); }
        // Mark as read immediately since user is in the chatroom
        DB.markConversationRead(convo.dbId, GS.user.id).catch(() => {});
      } else {
        // Own message echoed back — update status to delivered
  ...
  // Subscribe to read receipts — when recipient reads, update our sent messages to "read"
  React.useEffect(() => {
    if (!convo?.dbId || !isSupabaseReady() || !GS.user.id) return;
    const channel = DB.subscribeToConversation?.(convo.dbId, (updated) => {
      // Determine which unread field belongs to the OTHER user
      const isA = updated.user_a === GS.user.id;
      const otherUnread = isA ? updated.unread_b : updated.unread_a;
      // If the other user's unread went to 0, they've read our messages
      if (otherUnread === 0) {
        setMsgs(prev => prev.map(m => m.mine && m.status !== "read" ? { ...m, status: "read" } : m));
      }
    });
  ...
  // Clear unread when entering chatroom + mark read in DB
  useEffect(()=>{
    ...
    // Mark conversation as read in Supabase
    if(convo?.dbId&&isSupabaseReady()&&GS.user.id){
      DB.markConversationRead(convo.dbId, GS.user.id).catch(()=>{});
    }
  },[convo?.id]);
  ...
  useEffect(()=>{
    ...
    DB.getMessages(convo.dbId).then(dbMsgs=>{
      ...
      const mapped = dbMsgs.map(m=>{
        const isMine = m.sender_id===GS.user.id;
        let status = "sent";
        if(isMine){
          // For my messages: check if the other person has read it
      ...
      setMsgs(mapped);
      if(convo) { convo.msgs = mapped; saveGS(); }
    });
  },[convo?.dbId]);
  ...
  return <div style={{height:"100%",display:"flex",flexDirection:"column",background:T.bg}}>
    ...
    <div ref={scrollRef} className="toono-readable" style={{flex:1,overflowY:"auto",scrollbarWidth:"none",padding:"16px 20px"}}>
      ...
      {msgs.map((m,i)=>{
        ...
        return <div key={m.id} style={{display:"flex",justifyContent:m.mine?"flex-end":"flex-start",marginBottom:lastOfGroup?10:2,alignItems:"flex-end",gap:6}}>
          {/* Avatar only on last message of a left-side group */}
          {!m.mine&&(lastOfGroup
            ?<Avt size={28} color={convo?.accent||T.accent} photo={convo?.photo}/>
          ...
          <div style={{maxWidth:"72%"}}>
            {m.img
              ...
              :<div style={{background:m.mine?T.accent:T.s1,borderRadius:m.mine?`18px ${firstOfGroup?"18px":"6px"} ${lastOfGroup?"4px":"6px"} 18px`:`${firstOfGroup?"18px":"6px"} 18px 18px ${lastOfGroup?"4px":"6px"}`,padding:"10px 14px",border:m.mine?"none":`1px solid ${T.border}`}}>
                <div style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:14,color:m.mine?"#fff":T.textH,lineHeight:1.5,whiteSpace:"pre-wrap",wordBreak:"break-word"}}>{m.text}</div>
              </div>}
            {/* Timestamp only on last message of group */}
            {lastOfGroup&&<div style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:10,color:T.textSub,marginTop:3,textAlign:m.mine?"right":"left",display:"flex",alignItems:"center",justifyContent:m.mine?"flex-end":"flex-start",gap:2}}>
              {m.time}
