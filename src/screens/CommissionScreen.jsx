      ...
      try{
        ...
        if (GS.user.id && creatorId) {
          const commPayload = {
            ...
            attachments: await Promise.all(attachments.map(async (a, i) => {
              if (!a.url || !a.url.startsWith("data:")) return a.url;
              if (!isSupabaseReady() || !GS.user.id) return a.url;
              const path = `commissions/${GS.user.id}/${Date.now()}_${i}.jpg`;
              const uploaded = await DB.uploadFile("commissions", path, a.url).catch(() => null);
              return uploaded || a.url;
            })),
            status: "pending",
          };
          if (isSupabaseReady()) {
    ...
    <div style={{flex:1,overflowY:"auto",scrollbarWidth:"none",padding:"0 20px"}}>
      <Crd onClick={() => creatorId && nav("profile", {creatorId})} style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:12,marginBottom:20,cursor:"pointer"}}>
        ...
        <div>
          ...
          <div style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:12,color:T.textSub}}>{targetCreator.field || "Creator"}</div>
        </div>
      </Crd>
      {step===0&&<>
