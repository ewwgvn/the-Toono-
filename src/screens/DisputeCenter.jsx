...
export default function DisputeCenter({ nav, goBack, refresh }) {
  ...
  return <div style={{ height: "100%", display: "flex", flexDirection: "column", background: "#FFFFFF" }}>
    <div style={{ padding: "16px 16px 12px", display: "flex", alignItems: "center", gap: 12, borderBottom: `1px solid ${T.borderLight}` }}>
      ...
      <div style={{ fontFamily: F, fontSize: 16, fontWeight: 600, color: T.textH }}>Гомдол, буцаалт</div>
    </div>

    <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none", padding: "16px" }}>
      {/* Escrow info */}
      <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, padding: "14px 16px", marginBottom: 20 }}>
      ...
      {/* Existing disputes */}
      {disputes.length > 0 && <>
        <div style={{ fontFamily: F, fontSize: 14, fontWeight: 600, color: T.textH, marginBottom: 12 }}>Миний маргаан ({disputes.length})</div>
        {disputes.map(d => <Crd key={d.id} style={{ padding: "14px 16px", marginBottom: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: F, fontSize: 13, fontWeight: 600, color: T.textH, marginBottom: 2 }}>{d.orderTitle}</div>
              <div style={{ fontFamily: F, fontSize: 11, color: T.textSub }}>{d.typeLabel} · {d.date}</div>
            </div>
            <span style={{ fontFamily: F, fontSize: 11, fontWeight: 600, color: statusColors[d.status] || `${T.textSub}`, padding: "3px 8px", borderRadius: 20, background: T.s2 }}>
              {statusLabels[d.status] || d.status}
            </span>
          </div>
          {d.details && <div style={{ fontFamily: F, fontSize: 12, color: T.textSub, lineHeight: 1.5, marginTop: 6 }}>{d.details}</div>}
        </Crd>)}
        <div style={{ height: 1, background: T.borderLight, margin: "20px 0" }} />
      </>}

      {/* New dispute */}
      <div style={{ fontFamily: F, fontSize: 14, fontWeight: 600, color: T.textH, marginBottom: 14 }}>Шинэ маргаан нээх</div>

      {eligibleOrders.length === 0 ? (
        <div style={{ textAlign: "center", padding: "20px 0", fontFamily: F, fontSize: 13, color: T.textSub }}>
        ...
        <>
          {step === 0 && <>
            {/* Order selector */}
            <div style={{ fontFamily: F, fontSize: 12, fontWeight: 500, color: T.textSub, marginBottom: 8 }}>Захиалга сонгох</div>
            {eligibleOrders.map(o => <button type="button" key={o.id} onClick={() => setSelectedOrderId(o.id)} style={{
              width: "100%", background: selectedOrderId === o.id ? T.s2 : "#FFFFFF",
              border: `1px solid ${selectedOrderId === o.id ? T.textH : T.border}`,
              borderRadius: 8, padding: "12px 14px", cursor: "pointer",
              display: "flex", alignItems: "center", gap: 12, textAlign: "left", marginBottom: 8,
            }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: F, fontSize: 13, fontWeight: 600, color: T.textH }}>{o.title}</div>
                <div style={{ fontFamily: F, fontSize: 11, color: T.textSub }}>{o.creator} · ₮{(o.price || 0).toLocaleString()}</div>
              </div>
              {selectedOrderId === o.id && <IcCheck />}
            </button>)}

            <div style={{ marginTop: 16, fontFamily: F, fontSize: 12, fontWeight: 500, color: T.textSub, marginBottom: 8 }}>Маргааны төрөл</div>
            {types.map(t => <button type="button" key={t[0]} onClick={() => { setType(t[0]); setStep(1); }} style={{
              width: "100%", background: "#FFFFFF", border: `1px solid ${T.border}`,
              borderRadius: 8, padding: "12px 14px", cursor: "pointer",
              display: "flex", alignItems: "center", gap: 12, textAlign: "left", marginBottom: 8,
            }}>
              <div style={{ color: T.textH, flexShrink: 0 }}>{t[1]}</div>
              <div>
                <div style={{ fontFamily: F, fontSize: 13, fontWeight: 600, color: T.textH }}>{t[2]}</div>
                <div style={{ fontFamily: F, fontSize: 11, color: T.textSub, marginTop: 2 }}>{t[3]}</div>
              </div>
            </button>)}
          </>}
          ...
          {step === 1 && <>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontFamily: F, fontSize: 12, fontWeight: 500, color: T.textSub, marginBottom: 6 }}>Асуудлаа тайлбарлана уу</div>
              <textarea value={details} onChange={e => setDetails(e.target.value)} placeholder="Юу болсон талаар дэлгэрэнгүй бичнэ үү..." style={{
                width: "100%", minHeight: 120, background: "#FFFFFF", border: `1px solid ${T.border}`,
