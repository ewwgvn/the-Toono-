    ...
    <div style={{flex:1,padding:"24px 24px 40px",overflowY:"auto",scrollbarWidth:"none"}}>
      ...
      {mode==="login"&&<>
        ...
        <div style={{marginBottom:14}}>
          <label htmlFor="login-email" style={{display:"block",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:13,fontWeight:600,color:T.textSub,marginBottom:7}}>Имэйл</label>
          <input
          ...
          />
          {errors.email&&<div id="login-email-err" role="alert" style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:11,color:T.red,marginTop:5,display:"flex",alignItems:"center",gap:4}}><span style={{display:"flex"}}><IcWarning/></span>{errors.email}</div>}
        </div>

        {/* Password */}
        <div style={{marginBottom:6}}>
          <label htmlFor="login-pw" style={{display:"block",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:13,fontWeight:600,color:T.textSub,marginBottom:7}}>Нууц үг</label>
          <div style={{position:"relative"}}>
            <input
              id="login-pw" type={showPw?"text":"password"} value={pw} placeholder="••••••••"
              onChange={e => {setPw(e.target.value);setErrors(er => ({...er,pw:""}));}}
              onKeyDown={e => e.key==="Enter"&&handleLogin()}
              aria-invalid={!!errors.pw} aria-describedby={errors.pw?"login-pw-err":undefined}
            ...
            />
            <button type="button" aria-label={showPw?"Нууц үг нуух":"Нууц үг харуулах"} onClick={() => setShowPw(!showPw)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:T.textSub,fontSize:16,padding:2}}>{showPw?<IcEyeOff/>:<IcEye/>}</button>
          </div>
          {errors.pw&&<div id="login-pw-err" role="alert" style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:11,color:T.red,marginTop:5,display:"flex",alignItems:"center",gap:4}}><span style={{display:"flex"}}><IcWarning/></span>{errors.pw}</div>}
        </div>
      ...
      {mode==="signup"&&step===1&&<>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:24}}>
          {(role==="creator"
            ?[["Fashion Design",<IcFieldFashion key="fashion"/>],["Textile Design",<IcFieldTextile key="textile"/>],["Fine Art",<IcFieldArt key="art"/>],["Interior Design",<IcFieldDirection key="interior"/>],["Graphic Design",<IcFieldGraphic key="graphic"/>],["Photography",<IcFieldPhoto key="photo"/>],["3D Design",<IcField3D key="3d"/>],["Industrial Design",<IcFieldSpace key="industrial"/>],["Jewelry Design",<IcFieldJewelry key="jewelry"/>],["Ceramic",<IcFieldCeramic key="ceramic"/>],["Woodcraft",<IcFieldWood key="wood"/>],["Leatherwork",<IcFieldLeather key="leather"/>]]
            :[["Fashion Design",<IcFieldFashion key="fashion"/>],["Textile Design",<IcFieldTextile key="textile"/>],["Fine Art",<IcFieldArt key="art"/>],["Graphic Design",<IcFieldGraphic key="graphic"/>],["Photography",<IcFieldPhoto key="photo"/>],["3D Design",<IcField3D key="3d"/>],["Jewelry Design",<IcFieldJewelry key="jewelry"/>]]
          ).map(([label, icon]) => {
      ...
      {mode==="signup"&&step===2&&<>
        {/* Name */}
        <div style={{marginBottom:14}}>
          <label htmlFor="signup-name" style={{display:"block",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:13,fontWeight:600,color:T.textSub,marginBottom:7}}>Нэр / Хочоо</label>
          <input id="signup-name" value={name} onChange={e => {setName(e.target.value);setErrors(er => ({...er,name:""}));}} placeholder="Уугана Болд" aria-invalid={!!errors.name} aria-describedby={errors.name?"signup-name-err":undefined} style={{width:"100%",background:T.s1,border:`1.5px solid ${errors.name?T.red:T.border}`,borderRadius:13,padding:"13px 16px",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:14,color:T.textH,outline:"none",boxSizing:"border-box"}}/>
          {errors.name&&<div id="signup-name-err" role="alert" style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:11,color:T.red,marginTop:5}}>{errors.name}</div>}
        </div>
        {/* Email */}
        <div style={{marginBottom:14}}>
          <label htmlFor="signup-email" style={{display:"block",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:13,fontWeight:600,color:T.textSub,marginBottom:7}}>Имэйл</label>
          <input id="signup-email" type="email" value={email} onChange={e => {setEmail(e.target.value);setErrors(er => ({...er,email:""}));}} placeholder="name@email.com" aria-invalid={!!errors.email} aria-describedby={errors.email?"signup-email-err":undefined} style={{width:"100%",background:T.s1,border:`1.5px solid ${errors.email?T.red:T.border}`,borderRadius:13,padding:"13px 16px",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:14,color:T.textH,outline:"none",boxSizing:"border-box"}}/>
          {errors.email&&<div id="signup-email-err" role="alert" style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:11,color:T.red,marginTop:5}}>{errors.email}</div>}
        </div>
        {/* Password */}
        <div style={{marginBottom:14}}>
          <label htmlFor="signup-pw" style={{display:"block",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:13,fontWeight:600,color:T.textSub,marginBottom:7}}>Нууц үг</label>
          <div style={{position:"relative"}}>
            <input id="signup-pw" type={showPw?"text":"password"} value={pw} onChange={e => {setPw(e.target.value);setErrors(er => ({...er,pw:""}));}} placeholder="6-аас дээш тэмдэгт" aria-invalid={!!errors.pw} aria-describedby={errors.pw?"signup-pw-err":undefined} style={{width:"100%",background:T.s1,border:`1.5px solid ${errors.pw?T.red:T.border}`,borderRadius:13,padding:"13px 44px 13px 16px",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:14,color:T.textH,outline:"none",boxSizing:"border-box"}}/>
            <button type="button" aria-label={showPw?"Нууц үг нуух":"Нууц үг харуулах"} onClick={() => setShowPw(!showPw)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:T.textSub,fontSize:16}}>
              {showPw?<IcEyeOff/>:<IcEye/>}
            </button>
          </div>
          {pw.length>0&&<div aria-hidden="true" style={{display:"flex",gap:4,marginTop:6}}>
          ...
          {errors.pw&&<div id="signup-pw-err" role="alert" style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:11,color:T.red,marginTop:5}}>{errors.pw}</div>}
        </div>
        {/* Confirm */}
        <div style={{marginBottom:16}}>
          <label htmlFor="signup-pw-confirm" style={{display:"block",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:13,fontWeight:600,color:T.textSub,marginBottom:7}}>Нууц үг давтах</label>
          <input id="signup-pw-confirm" type="password" value={pwConfirm} onChange={e => {setPwConfirm(e.target.value);setErrors(er => ({...er,pwConfirm:""}));}} placeholder="Нууц үгийг дахин оруулна уу" aria-invalid={!!errors.pwConfirm} aria-describedby={errors.pwConfirm?"signup-pw-confirm-err":undefined} style={{width:"100%",background:T.s1,border:`1.5px solid ${errors.pwConfirm?T.red:pwConfirm&&pwConfirm===pw?T.green:T.border}`,borderRadius:13,padding:"13px 16px",fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:14,color:T.textH,outline:"none",boxSizing:"border-box"}}/>
          ...
          {errors.pwConfirm&&<div id="signup-pw-confirm-err" role="alert" style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:11,color:T.red,marginTop:5}}>{errors.pwConfirm}</div>}
        </div>
        {/* PIPA Consent */}
