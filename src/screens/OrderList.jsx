...
export default function OrderList({ nav, refresh, goBack }) {
  ...
  const statusMap = { "pending": "Хүлээгдэж байна", "making": "Хийж байна", "shipped": "Хүргэгдэж байна", "delivered": "Хүргэгдсэн", "done": "Дууссан", "cancelled": "Цуцлагдсан" };
  ...
  // Fetch orders from Supabase if local state is empty
  useEffect(() => {
    if (GS.orders.length > 0 || !isSupabaseReady() || !GS.user.id) return;
    setLoading(true);
    DB.getMyOrders(GS.user.id).then(data => {
      if (data?.length) { GS.orders = data; saveGS(); refresh(); }
  ...
  return (
    <Simple nav={nav} title="Захиалгын жагсаалт" back="me" goBack={goBack}>
      ...
      <div style={{ display: "flex", gap: 6, overflowX: "auto", scrollbarWidth: "none", marginBottom: 16 }}>
        {[["all", "Бүгд"], ["pending", "Хүлээгдэж"], ["making", "Хийж байна"], ["delivered", "Ирсэн"], ["done", "Дууссан"], ["cancelled", "Цуцлагдсан"]].map(([k, l]) =>
      ...
      {loading
        ? <div style={{ padding: "40px 0", textAlign: "center", fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 13, color: T.textSub }}>Уншиж байна...</div>
        : orders.length === 0
