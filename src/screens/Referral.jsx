...
export default function Referral({ nav, goBack }) {
  ...
  return <Simple nav={nav} title="Найзаа урих" back="me">
    {/* Banner */}
    <div style={{ background: `linear-gradient(135deg,${T.accent},#3A6FD8,#666666)`, borderRadius: 20, padding: "24px 20px", marginBottom: 20, textAlign: "center", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 22, fontWeight: 800, color: "#fff", marginBottom: 6 }}>Найз урих бүрд ₮10,000</div>
      <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 13, color: "rgba(255,255,255,0.8)", lineHeight: 1.6 }}>Найзаа урих бүрд та хоёулаа ₮10,000 тус бүр авна. Найзын эхний худалдан авалтын дараа шагнал ирнэ.</div>
    </div>

    {/* My code */}
    <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 13, fontWeight: 600, color: T.textSub, marginBottom: 10 }}>Миний урилгын код</div>
    <Crd style={{ padding: "16px", marginBottom: 20 }}>
      ...
      }} style={{ marginTop: 8 }}><span style={{ display: "flex", marginRight: 4 }}><IcLink /></span>Холбоос хуваалцах</PBtn>
    </Crd>
    ...
    <Crd style={{ display: "flex", marginBottom: 20 }}>
      {[["3", "Урьсан найз"], ["2", "Баталгаажсан"], ["₮" + earned.toLocaleString(), "Нийт шагнал"]].map((s, i) => <div key={i} style={{ flex: 1, textAlign: "center", padding: "16px 0", borderRight: i < 2 ? `1px solid ${T.border}` : "none" }}>
        <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: i === 2 ? 16 : 20, fontWeight: 800, color: i === 2 ? T.green : T.textH }}>{s[0]}</div>
        <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 10, color: T.textSub, marginTop: 4 }}>{s[1]}</div>
      </div>)}
    </Crd>
    ...
    {[["1", "Кодоо хуваалц", "Найздаа код эсвэл холбоос илгээ"], ["2", "Найз бүртгүүлнэ", "Найзынхаа Үлгэр бүртгэл нээхэд ашиглана"], ["3", "Хоёулаа шагнал авна", "Найзынхаа эхний худалдан авалтын дараа ₮10,000 тус бүр авна"]].map(s => <div key={s[0]} style={{ display: "flex", gap: 14, marginBottom: 14, alignItems: "flex-start" }}>
      ...
      <div>
        ...
        <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 12, color: T.textSub }}>{s[2]}</div>
      </div>
    </div>)}

    <div style={{ height: 1, background: T.border, margin: "16px 0" }} />
    <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 13, fontWeight: 700, color: T.textH, marginBottom: 10 }}>Урьсан найзуудын жагсаалт</div>
    {history.map((h, i) => <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: `1px solid ${T.border}` }}>
      <div>
        <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 13, fontWeight: 600, color: T.textH }}>{h.name}</div>
        <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 11, color: T.textSub }}>{h.date}</div>
      </div>
      <div style={{ textAlign: "right" }}>
        <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 13, fontWeight: 700, color: h.reward > 0 ? T.green : T.textSub }}>{h.reward > 0 ? "+₮" + h.reward.toLocaleString() : "–"}</div>
        <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 10, color: T.textSub }}>{h.status}</div>
      </div>
    </div>)}
  </Simple>;
}
