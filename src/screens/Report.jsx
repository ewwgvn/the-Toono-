...
export default function Report({ nav, goBack }) {
  ...
  return <div style={{ height: "100%", display: "flex", flexDirection: "column", background: T.bg }}>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "0 20px" }}>
      ...
      {types.map(t => <button key={t} onClick={() => setType(t)} style={{ width: "100%", textAlign: "left", background: type === t ? T.accentSub : T.s1, border: `1.5px solid ${type === t ? T.accent : T.border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 8, fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 14, color: type === t ? T.accent : T.textH, cursor: "pointer" }}>{t}</button>)}
