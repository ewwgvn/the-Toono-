...
export default function Upload({ nav, goBack }) {
  ...
  if (GS.currentRole !== "creator") {
    return <div style={{height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"#FFFFFF",padding:32}}>
      <div style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:16,fontWeight:600,color:T.textH,marginBottom:8}}>Бүтээлч эрхээр нэвтэрнэ үү</div>
      <div style={{fontFamily:"'Helvetica Neue', Arial, sans-serif",fontSize:13,color:T.textSub,marginBottom:20,textAlign:"center"}}>Бүтээл байршуулахын тулд бүтээлч бүртгэлтэй байх шаардлагатай</div>
      <PBtn onClick={() => goBack ? goBack() : nav("home")}>Буцах</PBtn>
    </div>;
  }

  // Image picker — compress then immediately upload to Supabase Storage
  const handleImagePick = (e) => {
    const files = Array.from(e.target.files || []);
    const valid = files.filter(f => f.type.startsWith("image/")).slice(0, 10 - mediaFiles.length);
    valid.forEach((f, fileIdx) => {
      const reader = new FileReader();
      reader.onload = async ev => {
        const compressed = await compressImage(ev.target.result, 1200, 0.82);
        const _tid = Date.now() + fileIdx + Math.random();

        if (isSupabaseReady() && GS.user.id) {
          // Add with base64 preview + uploading flag
          setMediaFiles(prev => [...prev, { type: "image", url: compressed, name: f.name, uploading: true, _tid }]);
          const path = `works/${GS.user.id}/${Date.now()}_${fileIdx}.jpg`;
          const publicUrl = await DB.uploadFile("works", path, compressed);
          if (publicUrl) {
            // Swap base64 preview → Storage URL
            setMediaFiles(prev => prev.map(m => m._tid === _tid ? { type: "image", url: publicUrl, name: f.name } : m));
          } else {
            // Upload failed — remove from list (DB.uploadFile already toasts)
            setMediaFiles(prev => prev.filter(m => m._tid !== _tid));
          }
        } else {
          // Supabase offline — keep base64 as fallback
      ...
      {step === 0 && <>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
            {mediaFiles.map((f, idx) => <div key={f._tid ?? idx} style={{ position: "relative", aspectRatio: "1", borderRadius: 12, overflow: "hidden", background: T.s2 }}>
              <img src={f.url} alt="" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: f.uploading ? 0.4 : 1 }} onClick={() => { if (!f.uploading) { setCropWorkIdx(idx); setCropWorkSrc(f.url); } }} />

              {/* Per-image upload spinner */}
              {f.uploading && <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, pointerEvents: "none" }}>
                ...
                <span style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", fontSize: 9, fontWeight: 600, color: "#fff" }}>Байршуулж байна...</span>
              </div>}
