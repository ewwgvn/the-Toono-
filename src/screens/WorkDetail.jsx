  ...
  return <div style={{height:"100%",display:"flex",flexDirection:"column",background:T.bg}}>
    ...
    <div style={{padding:"16px 16px 10px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${T.s2}`}}>
      <button type="button" onClick={()=>goBack?goBack():nav("home")} className="toono-pressable" style={{display:"flex",alignItems:"center",gap:8,background:"none",border:"none",cursor:"pointer",color:T.textH,padding:"4px 0"}}>
        <IcBack/>
        <span style={{fontFamily:F,fontSize:15,fontWeight:600,color:T.textH}}>Буцах</span>
      </button>
    ...
    <div style={{flex:1,overflowY:"auto",scrollbarWidth:"none"}}>
      ...
      {zoomOpen&&<div onClick={()=>setZoomOpen(false)} style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.95)",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}>
        ...
        <div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.5)",marginTop:10}}>{(imgIdx||0)+1} / {w.images.length} · Зүүн баруун шудрах</div>
      </div>}
      ...
      {w.images?.length>0
        ?<div style={{margin:"0 20px",borderRadius:20,overflow:"hidden",position:"relative",cursor:"zoom-in",background:T.s2}} onClick={()=>setZoomOpen(true)}
          ...
          {w.badge&&<span style={{position:"absolute",top:14,left:14,fontFamily:F,fontSize:12,fontWeight:700,color:"#fff",background:T.accent,padding:"5px 12px",borderRadius:10}}>{w.badge}</span>}
          {w.images.length>1&&<div style={{position:"absolute",bottom:14,right:14,background:"rgba(0,0,0,0.5)",borderRadius:10,padding:"6px 12px",display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontFamily:F,fontSize:12,color:"#fff"}}>{(imgIdx||0)+1}/{w.images.length}</span>
          </div>}
        </div>
        :<div style={{height:260,background:`linear-gradient(135deg,${T.accent},${T.accentHover},${T.accent})`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",margin:"0 20px",borderRadius:20}}>
          ...
          <div style={{position:"absolute",bottom:14,right:14,background:"rgba(0,0,0,0.5)",borderRadius:10,padding:"6px 12px"}}>
            <span style={{fontFamily:F,fontSize:12,color:T.textB}}><IcHeart/> {(w.likes||0).toLocaleString()}</span>
          </div>
        </div>}
      {/* Video if available */}
      ...
      <div className="wd-info" style={{padding:"18px 20px 0"}}>
        <div style={{marginBottom:12}}>
          <div style={{fontFamily:F,fontSize:22,fontWeight:800,color:T.textH,lineHeight:1.2,letterSpacing:"-0.02em",marginBottom:6}}>{w.title}</div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{display:"inline-block",width:7,height:7,background:T.accent,transform:"rotate(45deg)"}}/>
            <span style={{fontFamily:F,fontSize:24,fontWeight:800,color:T.accent,letterSpacing:"-0.01em"}}>{fmtP(w)}</span>
          </div>
        </div>
        ...
        <Crd onClick={()=>nav("profile",{creatorId:w.creator_id})} style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,marginBottom:18}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontFamily:F,fontSize:14,fontWeight:700,color:T.textH}}>{creatorName}</div>
            <div style={{fontFamily:F,fontSize:12,color:T.textSub}}>{w.cat||"Бүтээлч"}</div>
          </div>
          <span style={{color:T.textSub}}><IcChevron/></span>
        </Crd>
        <div style={{height:1,background:T.border}}/>
        <div style={{padding:"16px 0"}}>
          <div style={{fontFamily:F,fontSize:11,fontWeight:700,letterSpacing:'0.22em',textTransform:'uppercase',color:T.accent,marginBottom:10}}>ТАЙЛБАР</div>
          <div style={{fontFamily:F,fontSize:14,color:T.textB,lineHeight:1.8}}>{w.description||w.desc||""}</div>
        </div>
        {/* ── 스펙 칩 인라인 미리보기 ── */}
        {(w.medium||w.sizes?.length>0||w.duration)&&<div style={{marginBottom:8}}>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {w.medium&&w.medium!=="—"&&<span style={{fontFamily:F,fontSize:11,fontWeight:500,color:"#555",background:T.s2,padding:"4px 10px",borderRadius:20,border:`1px solid ${T.border}`}}>{w.medium}</span>}
            {w.sizes?.map(s=><span key={s} style={{fontFamily:F,fontSize:11,fontWeight:500,color:"#555",background:T.s2,padding:"4px 10px",borderRadius:20,border:`1px solid ${T.border}`}}>{s}</span>)}
            {w.duration&&<span style={{fontFamily:F,fontSize:11,fontWeight:500,color:"#555",background:T.s2,padding:"4px 10px",borderRadius:20,border:`1px solid ${T.border}`}}>{w.duration}</span>}
            {w.stock>0&&<span style={{fontFamily:F,fontSize:11,fontWeight:500,color:"#555",background:T.s2,padding:"4px 10px",borderRadius:20,border:`1px solid ${T.border}`}}>{w.stock}ш</span>}
          </div>
        </div>}
      </div>{/* /wd-info */}
      </div>{/* /wd-grid */}
      <div className="toono-readable" style={{padding:"0 20px"}}>
        ...
        <div style={{marginTop:4,marginBottom:24,borderTop:`1px solid ${T.borderLight}`,paddingTop:16}}>
          <div style={{fontFamily:F,fontSize:30,fontWeight:700,color:T.textH,marginBottom:12,lineHeight:1}}>Сэтгэгдэл {comments.length>0?`(${comments.length})`:""}</div>
          {GS.user.id && <div style={{display:"flex",gap:10,marginBottom:16,alignItems:"center"}}>
            ...
            <input value={commentInput} onChange={e=>setCommentInput(e.target.value)}
              ...
              style={{flex:1,background:T.s2,border:`1px solid ${T.border}`,borderRadius:20,padding:"10px 14px",fontFamily:F,fontSize:13,color:T.textH,outline:"none"}}/>
            {commentInput.trim() && <PBtn small onClick={submitComment}>Илгээх</PBtn>}
          </div>}
          {comments.length===0
            ?<div style={{fontFamily:F,fontSize:13,color:T.textSub,textAlign:"center",padding:"12px 0"}}>Сэтгэгдэл байхгүй</div>
            :comments.map(c=><div key={c.id} style={{display:"flex",gap:10,padding:"10px 0",borderBottom:`1px solid ${T.borderLight}`}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontFamily:F,fontSize:12,fontWeight:600,color:T.textH,marginBottom:2}}>{c.profiles?.name||"User"}</div>
                <div style={{fontFamily:F,fontSize:13,color:T.textB,lineHeight:1.5}}>{c.text}</div>
              </div>
            </div>)}
        </div>
        {(()=>{
          ...
          return <div style={{marginTop:4}}>
            ...
            <div style={{display:"flex",gap:12,overflowX:"auto",scrollbarWidth:"none"}}>
              {more.map(ww=><div key={ww.id} onClick={()=>nav("work",{workId:ww.id})} className="toono-card-tap" style={{flexShrink:0,width:130,cursor:"pointer"}}>
                ...
                <div style={{fontFamily:F,fontSize:12,fontWeight:600,color:T.textH,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ww.title}</div>
                <div style={{fontFamily:F,fontSize:12,color:T.textH,fontWeight:700}}>{fmtP(ww)}</div>
              </div>)}
            </div>
          </div>;
        })()}
    ...
    <BottomSheet open={offerOpen} onClose={()=>setOfferOpen(false)} title="Үнийн санал илгээх" height="50%">
      <div style={{fontFamily:F,fontSize:13,color:T.textSub,marginBottom:14}}>Бүтээлчид хүссэн үнэ санал болгоно. Хариуг хүлээнэ үү.</div>
      <div style={{fontFamily:F,fontSize:12,fontWeight:600,color:T.textSub,marginBottom:6}}>Анхны үнэ: {fmtP(w)}</div>
      <input type="number" value={offerPrice} onChange={e=>setOfferPrice(e.target.value)} placeholder="Санал болгох үнэ (₮)" style={{width:"100%",background:T.s2,border:`1px solid ${T.border}`,borderRadius:13,padding:"14px 16px",fontFamily:F,fontSize:16,fontWeight:700,color:T.textH,outline:"none",marginBottom:12,boxSizing:"border-box"}}/>
      <textarea value={offerMsg} onChange={e=>setOfferMsg(e.target.value)} placeholder="Зурвас (заавал биш)" rows={3} style={{width:"100%",background:T.s2,border:`1px solid ${T.border}`,borderRadius:13,padding:"12px 16px",fontFamily:F,fontSize:14,color:T.textH,outline:"none",resize:"none",marginBottom:16,boxSizing:"border-box"}}/>
    ...
    {(w.sizes?.length>0||w.colors?.length>0)&&<div style={{padding:"8px 20px 0",background:T.bg}}>
      {w.sizes?.length>0&&<div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}>
        <span style={{fontFamily:F,fontSize:11,color:T.textSub,width:"100%",marginBottom:2}}>Хэмжээ</span>
        {w.sizes.map(s=><button type="button" key={s} onClick={()=>setSelSize(s)} style={{padding:"5px 12px",borderRadius:8,cursor:"pointer",fontFamily:F,fontSize:12,fontWeight:600,background:selSize===s?T.accent:T.s1,border:`1px solid ${selSize===s?T.accent:T.border}`,color:selSize===s?"#fff":T.textSub}}>{s}</button>)}
      </div>}
      {w.colors?.length>0&&<div style={{display:"flex",gap:7,marginBottom:8,alignItems:"center"}}>
        <span style={{fontFamily:F,fontSize:11,color:T.textSub}}>Өнгө</span>
    ...
    <div className="toono-readable" style={{padding:"10px 16px calc(14px + env(safe-area-inset-bottom,0px))",background:T.bg,borderTop:`1px solid ${T.border}`,display:"flex",gap:8,alignItems:"center"}}>
      ...
      {w.price>0&&(w.stock===undefined||w.stock>0)&&!(w.creator_id===GS.user.id||w.cid===GS.user.id)&&<div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
        ...
        <span style={{fontFamily:F,fontSize:14,fontWeight:700,color:T.textH,minWidth:20,textAlign:"center"}}>{qty}</span>
      ...
      {(w.creator_id===GS.user.id||w.cid===GS.user.id) ? (
        <div style={{flex:1,textAlign:"center",fontFamily:F,fontSize:13,color:T.textSub}}>Энэ таны бүтээл</div>
      ...
      /* 판매 가능한 작품 */
        ...
        <>
          ...
          <button type="button" className="toono-pressable" onClick={()=>{
            ...
            nav("checkout");
          }} style={{flex:1,background:T.accent,border:"none",borderRadius:14,height:42,fontFamily:F,fontSize:14,fontWeight:700,color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6,whiteSpace:"nowrap",overflow:"hidden",minWidth:0}}>
      ...
      /* 품절 */
        ...
        <button type="button" className="toono-pressable" onClick={()=>{toast("Нөөц ирэхэд мэдэгдэл илгээнэ","success");}} style={{flex:1,background:T.s2,border:`1px solid ${T.border}`,borderRadius:14,height:42,fontFamily:F,fontSize:14,fontWeight:600,color:T.textSub,cursor:"pointer"}}>Нөөц ирэхэд мэдэгдэх</button>

      /* 커미션 전용 */
      ) : (
        <button type="button" className="toono-pressable" onClick={()=>nav("commission",{creatorId:w.creator_id})} style={{flex:1,background:T.accent,border:"none",borderRadius:14,height:42,fontFamily:F,fontSize:15,fontWeight:700,color:"#fff",cursor:"pointer"}}>Захиалга өгөх</button>
      )}
    </div>
  </div>;
}
