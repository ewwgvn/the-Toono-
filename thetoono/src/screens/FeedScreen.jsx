...
export default function FeedScreen({ nav, refresh, goBack }) {
  ...
  const stories = [
    { id: GS.user.id, name: "Миний", photo: GS.user.photo, accent: "#5B8FE8", _self: true },
  ...
  const FeedPost = ({ w, creator }) => {
    ...
    return <div style={{ marginBottom: 2 }}>
      ...
      <div style={{ padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
        <div onClick={() => creator?.id && nav("profile", { creatorId: creator.id })} style={{ width: 34, height: 34, borderRadius: 12, background: (creator?.accent || "#5B8FE8") + "30", border: `2px solid ${creator?.accent || "#5B8FE8"}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden", flexShrink: 0 }}>
          {creator?.photo ? <img src={creator.photo} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <Toono size={20} color={creator?.accent || "#5B8FE8"} />}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: "system-ui", fontSize: 13, fontWeight: 700, color: T.textH, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{creator?.name || w.creator || "—"}</div>
          <div style={{ fontFamily: "system-ui", fontSize: 11, color: T.textSub }}>{creator?.field || ""}</div>
        </div>
        <button onClick={() => nav("work", { workId: w.id })} style={{ background: T.accentSub, border: `1px solid ${T.accentGlow}`, borderRadius: 8, padding: "5px 12px", fontFamily: "system-ui", fontSize: 11, fontWeight: 700, color: T.accent, cursor: "pointer", flexShrink: 0 }}>{fmtP(w)}</button>
        <button style={{ background: "none", border: "none", color: T.textSub, cursor: "pointer", display: "flex", padding: 4 }}><IcDots /></button>
      </div>

      {/* Image */}
      <div onClick={() => nav("work", { workId: w.id })} style={{ width: "100%", aspectRatio: "1/1", background: img ? T.s2 : `linear-gradient(145deg,${w.accent || "#5B8FE8"}18,${w.accent || "#5B8FE8"}06)`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden", position: "relative" }}>
        {img
          ...
          : <Toono size={80} color={w.accent || "#5B8FE8"} />}
        ...
        {w.price > 0 && <div style={{ position: "absolute", top: 12, right: 12, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(8px)", borderRadius: 10, padding: "4px 10px" }}>
          <span style={{ fontFamily: "system-ui", fontSize: 12, fontWeight: 700, color: "#fff" }}>{fmtP(w)}</span>
        </div>}
      </div>
      ...
      <div style={{ padding: "0 14px 14px" }}>
        {w.likes_count > 0 && <div style={{ fontFamily: "system-ui", fontSize: 12, fontWeight: 700, color: T.textH, marginBottom: 4 }}>{(w.likes_count || 0).toLocaleString()} хүнд таалагдав</div>}
        <div style={{ fontFamily: "system-ui", fontSize: 13, color: T.textH, lineHeight: 1.6 }}>
        ...
        {w.tags?.length > 0 && <div style={{ marginTop: 4, display: "flex", flexWrap: "wrap", gap: 4 }}>
          {w.tags.slice(0, 4).map(t => <span key={t} style={{ fontFamily: "system-ui", fontSize: 11, color: T.accent }}>#{t}</span>)}
        </div>}
        <button onClick={() => nav("work", { workId: w.id })} style={{ marginTop: 8, width: "100%", background: T.accent, border: "none", borderRadius: 10, padding: "9px", fontFamily: "system-ui", fontSize: 13, fontWeight: 700, color: "#fff", cursor: "pointer" }}>Авах</button>
      </div>
    </div>;
  };
  ...
  return <div style={{ height: "100%", display: "flex", flexDirection: "column", background: T.bg }}>
    ...
    <div style={{ padding: "16px 16px 10px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${T.border}`, flexShrink: 0 }}>
      <div style={{ fontFamily: "Georgia,serif", fontSize: 24, fontWeight: 700, color: T.textH, letterSpacing: "-.5px" }}>toono</div>
    ...
    <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none" }}>
        ...
        <div style={{ display: "flex", gap: 14, overflowX: "auto", scrollbarWidth: "none", padding: "0 14px" }}>
          {stories.map((s, i) => <div key={s.id || i} onClick={() => s._self ? null : nav("profile", { creatorId: s.id })} style={{ flexShrink: 0, textAlign: "center", cursor: s._self ? "default" : "pointer", width: 62 }}>
            <div style={{ position: "relative", width: 58, height: 58, margin: "0 auto 5px" }}>
              {/* Gradient ring */}
              <div style={{ position: "absolute", inset: -2, borderRadius: 20, background: s._self ? "none" : `linear-gradient(135deg,${s.accent || "#5B8FE8"},#E040FB)`, padding: 2 }}>
                <div style={{ width: "100%", height: "100%", borderRadius: 18, background: T.bg }} />
              </div>
              <div style={{ position: "absolute", inset: 2, borderRadius: 16, background: (s.accent || "#5B8FE8") + "22", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {s.photo ? <img src={s.photo} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <Toono size={28} color={s.accent || "#5B8FE8"} />}
              </div>
              {/* Add button for self */}
            ...
            <div style={{ fontFamily: "system-ui", fontSize: 10, color: T.textSub, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 58 }}>{s._self ? "Таны" : (s.name || "—").split(" ")[0]}</div>
          </div>)}
          {/* Discover more */}
          <div onClick={() => nav("explore")} style={{ flexShrink: 0, textAlign: "center", cursor: "pointer", width: 62 }}>
            ...
            <div style={{ fontFamily: "system-ui", fontSize: 10, color: T.textSub }}>Дагах</div>
          </div>
        </div>
      </div>
      ...
      {feedWorks.length === 0
        ? <div style={{ textAlign: "center", padding: "60px 20px" }}>
          ...
          <div style={{ fontFamily: "system-ui", fontSize: 17, fontWeight: 700, color: T.textH, marginBottom: 8 }}>Фийд хоосон байна</div>
          <div style={{ fontFamily: "system-ui", fontSize: 13, color: T.textSub, lineHeight: 1.6, marginBottom: 20 }}>Бүтээлчийг дагаад тэдний шинэ<br />бүтээлийг энд харна уу</div>
          <PBtn onClick={() => nav("explore")}>Бүтээлч хайх</PBtn>
        </div>
        : feedWorks.map(w => {
          const creator = allCreators.find(c => c.id === w.creator_id) || { name: w.creator, photo: w.profiles?.photo || null, accent: w.accent || "#5B8FE8" };
