...
export default function Notifications({ nav, refresh }) {
  ...
  return (
    <Simple nav={nav} title={"Мэдэгдэл" + (unread > 0 ? " (" + unread + ")" : "")} back="me" right={unread > 0 && <button onClick={markAll} style={{ background: "none", border: "none", fontFamily: "system-ui", fontSize: 13, fontWeight: 600, color: T.accent, cursor: "pointer" }}>Бүгдийг уншсан</button>}>
      {notis.length === 0
          ...
          <div key={n.id} onClick={() => markOne(n)} style={{ padding: "14px 0", display: "flex", gap: 14, alignItems: "flex-start", borderBottom: `1px solid ${T.border}`, cursor: "pointer", transition: "background .15s" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <div style={{ fontFamily: "system-ui", fontSize: 14, fontWeight: n.read ? 500 : 700, color: n.read ? T.textSub : T.textH }}>{n.title}</div>
                <div style={{ fontFamily: "system-ui", fontSize: 11, color: T.textSub }}>{n.time}</div>
              </div>
              <div style={{ fontFamily: "system-ui", fontSize: 13, color: n.read ? T.textSub : T.textB, lineHeight: 1.5 }}>{n.desc}</div>
            </div>
